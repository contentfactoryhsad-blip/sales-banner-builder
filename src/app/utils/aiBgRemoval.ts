...
/**
 * 이 밝기 아래로는 건드리지 않는다. 그림자의 가장 짙은 부분이 대체로 여기쯤이고,
 * 더 내리면 제품의 어두운 가장자리까지 먹는다.
 */
const SHADOW_MIN = 170;
/** 무채색 판정 — 채널 편차가 이보다 크면 제품 색으로 본다 (그림자는 회색이다) */
const SHADOW_NEUTRAL = 22;

/**
 * 배경에 남은 **부드러운 그림자**를 지운다. 기본 누끼는 건드리지 않는 별도 동작이다.
 *
 * 기본 누끼는 "흰색(243 이상) + 테두리에서 이어진 곳"만 지운다. 그림자는 흰색에서
 * 회색으로 번지는 구간이라 어디에 선을 긋든 그 아래가 덩어리로 남는다.
 *
 * 여기서는 **이미 지워진 배경에서 번져 나가며**, 회색이고 충분히 밝은 픽셀만 따라간다.
 * 잘라내지 않고 밝기에 따라 알파를 깎아 서서히 사라지게 한다 —
 * 243 이상은 완전 투명, 170 이하는 그대로, 사이는 비례.
 *
 * 이미 지워진 배경과 이어진 곳만 보므로 제품 안쪽은 건드리지 않고,
 * 제품 색(채도 있는 픽셀)이나 어두운 가장자리에서 번지기를 멈춘다.
 *
 * @returns 손댄 픽셀 수
 */
export function eraseSoftShadow(img: ImageData): number {
  const { width: w, height: h, data } = img;
  const N = w * h;
  const seen = new Uint8Array(N);
  const stack: number[] = [];

  // 이미 투명한 곳(지워진 배경)을 씨앗으로
  for (let i = 0; i < N; i++) {
    if (data[i * 4 + 3] < ALPHA_MIN) { seen[i] = 1; stack.push(i); }
  }
  ...
  while (stack.length) {
    ...
    for (const [dx, dy] of [[1, 0], [-1, 0], [0, 1], [0, -1]] as const) {
      ...
      // 회색이고 충분히 밝은 곳만 따라간다 — 제품 색·어두운 가장자리에서 멈춘다
      if (mx - mn > SHADOW_NEUTRAL || mn < SHADOW_MIN) continue;
      seen[j] = 1;
      stack.push(j);
      // 밝을수록 많이 깎는다 (243 이상 완전 투명 → 170 그대로)
      const t = Math.min(1, Math.max(0, (mn - SHADOW_MIN) / (WHITE_MIN - SHADOW_MIN)));
      const next = Math.round(data[p + 3] * (1 - t));
      if (next !== data[p + 3]) { data[p + 3] = next; touched++; }
    }
  }
  return touched;
}
